...
public class JwtTokenProvider {
    ...
    // 토큰 유효시간 (otp 포함)
    private long otpTokenValidTime = 10 * 1000L;
    ...
    // JWT 토큰 생성 
    public String createToken(String username, Map<?, ?> roles) {
        Claims claims = Jwts.claims().setSubject(username); // JWT payload 에 저장되는 정보단위, 보통 여기서 user를 식별하는 값을 넣는다.
        Date now = new Date();
        Boolean hasOtp = roles != null && roles.containsKey("otp");
        claims.put("roles", roles); // 정보는 key / value 쌍으로 저장된다.
        return Jwts.builder()
                .setClaims(claims) // 정보 저장
                .setIssuedAt(now) // 토큰 발행 시간 정보
                .setExpiration(new Date(now.getTime() + (hasOtp ? otpTokenValidTime : tokenValidTime))) // set Expire Time
    ...
    public String createRefreshToken(String token) {
        try {
            ...
            if (now.before(expire)){
                return createToken(claims.getSubject(), null);
            }
        } catch (Exception e) {
        }
        return "";
